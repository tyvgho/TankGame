...
def modifie_taille_image(nom_image, nouvelle_largeur, nouvelle_hauteur, conserver_proportions=False, smooth=True):
    """ change la taille d'une image (avant de l'afficher)
        Si conserver_proportions est True, l'image conservera ses proportions d'origine
        Si smooth est True, utilise smoothscale pour un meilleur rendu visuel
    """
    if not(nom_image in __images):
        charge_image(nom_image)
    ...
    if conserver_proportions:
        ...
        largeur_originale = __images_original[nom_image].get_width()
        hauteur_originale = __images_original[nom_image].get_height()
        ratio = min(nouvelle_largeur / largeur_originale, nouvelle_hauteur / hauteur_originale)
        nouvelle_largeur_prop = int(largeur_originale * ratio)
        nouvelle_hauteur_prop = int(hauteur_originale * ratio)
        
        # Utiliser les nouvelles dimensions proportionnelles
        if smooth and pygame.transform.get_smoothscale_backend() != "GENERIC":
            __images[nom_image] = pygame.transform.smoothscale(__images_original[nom_image], 
                                                             (nouvelle_largeur_prop, nouvelle_hauteur_prop))
        else:
            __images[nom_image] = pygame.transform.scale(__images_original[nom_image], 
                                                       (nouvelle_largeur_prop, nouvelle_hauteur_prop))
    else:
        # Redimensionnement sans conserver les proportions
        if smooth and pygame.transform.get_smoothscale_backend() != "GENERIC":
            __images[nom_image] = pygame.transform.smoothscale(__images_original[nom_image], 
...
def affiche_texte(texte : str, position : tuple, couleur : pygame.Color, taille_police = __POLICE_SIZE,police = __POLICE_NAME):
